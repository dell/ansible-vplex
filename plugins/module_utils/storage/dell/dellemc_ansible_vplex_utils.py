...
def config_vplexapi(module_params):
    ...
    vplex_timeout = module_params['vplex_timeout']
    cert = module_params['verifycert']
    ssl_cert = module_params['ssl_ca_cert']
    debug = module_params['debug']
    ...
    if not vplex_timeout:
        # default vplex timeout, it's already set, added as a precaution
        vplex_timeout = 30
    elif not 0 < vplex_timeout < 3601:
        msg = "Invalid vplex timeout value. The valid range is from 1 to 3600."
        return 400, msg
    ...
    try:
        ...
        if config.verify_ssl:
            ...
            msg = ("After timeout of {0} seconds and few retries, "
                   "Could not establish connection with "
                   "Host {1}".format(vplex_timeout, host))
...
def get_vplex_management_host_parameters():
    ...
    return dict(
        ...
        vplex_timeout=dict(type='int', required=False, default=30)
    )
...
def get_vplex_setup(vplexclient):
    ...
    except (ApiException, TypeError, ValueError) as ex:
        e_msg = "Could not find version of given setup, due to error "
        e_msg = e_msg + ver[0]['info'] or ex
        return e_msg
...
def validate_name(name, char_len, field):
    ...
    if name is None:
        msg = "Required {0} can not be empty".format(field)
        return False, msg
...
This method checks the type of exception and returns the valid error message

returns Valid error message based on the exception
'''


def display_error(err_msg, msg):
    """To exit the module with failure"""
    if type(msg) in (ValueError, TypeError):
        err_msg = err_msg.format(msg)
    else:
        err_msg = err_msg.format(error_msg(msg))
    return err_msg
