...
version_added: '1.2.0'
short_description:  Manage distributed device on VPLEX Storage System
description:
- Provisioning the distributed device on VPLEX Storage System includes
...
dist_device_details:
    ...
    returned: When distributed device exists in VPLEX
    type: complex
    contains:
        capacity:
...
class VplexDistributedDevice():
    ...
    def get_distributed_device(self, distributed_device_name):
        ...
        try:
            ...
            err_msg = 'Could not get the distributed device {0}'
            err_msg = err_msg.format(
                distributed_device_name) + ' due to error: {0}'
    ...
    def create_distributed_device(self, dist_device_create_payload,
        ...
        try:
            ...
            err_msg = 'Could not create the distributed device {0}'
            err_msg = err_msg.format(
                dist_device_create_payload['name']) + ' due to error: {0}'
            e_msg = utils.display_error(err_msg, err)
            LOG.error("%s\n%s\n", e_msg, err)
            self.module.fail_json(msg=e_msg)
    ...
    def update_distributed_device(
        ...
        try:
            ...
            err_msg = 'Could not update the distributed device {0}'
            err_msg = err_msg.format(
                distributed_device_name) + ' due to error: {0}'
            e_msg = utils.display_error(err_msg, err)
            LOG.error("%s\n%s\n", e_msg, err)
    ...
    def delete_distributed_device(self, distributed_device_name):
        ...
        try:
            ...
            err_msg = 'Could not delete the distributed device {0}'
            err_msg = err_msg.format(
                distributed_device_name) + ' due to error: {0}'
            e_msg = utils.display_error(err_msg, err)
            LOG.error("%s\n%s\n", e_msg, err)
    ...
    def is_device_present(self, cluster_name, device_name):
        ...
        try:
            ...
            err_msg = 'Could not find the device {0} in {1}'
            err_msg = err_msg.format(
                device_name, cluster_name) + ' due to error: {0}'
            e_msg = utils.display_error(err_msg, err)
            LOG.error("%s\n%s\n", e_msg, err)
            self.module.fail_json(msg=e_msg)
